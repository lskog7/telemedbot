...
class Requests:
    ...
    @staticmethod
    def users_in_db(telegram_id, key):
        count = Users.select(Users.telegram_id).where(Users.telegram_id == encrypt(telegram_id, key)).count()
        return count

    # Запись новой строки в таблицу users
    @staticmethod
    def write_user(telegram_id, key):
        # Записываем новую строчку в БД
        query = Users(telegram_id=encrypt(telegram_id, key), sex=0)
        query.save()

    # Возвращает пол выбранного пользователя
    @staticmethod
    def get_user_sex(telegram_id, key):
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        return Users.SEX_CHOICES[query[0].sex][1]

    # Возвращает имя выбранного пользователя
    @staticmethod
    def get_user_name(telegram_id, key):
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        return decrypt(query[0].name, key)

    # Возвращает фамилию выбранного пользователя
    @staticmethod
    def get_user_surname(telegram_id, key):
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        return decrypt(query[0].surname, key)

    # Возвращает отчество выбранного пользователя
    @staticmethod
    def get_user_patronymic(telegram_id, key):
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        return decrypt(query[0].patronymic, key)

    # Возвращает дату рождения выбранного пользователя
    @staticmethod
    def get_user_b_date(telegram_id, key):
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        return query[0].b_date

    # Сохраняет имя пользователя
    @staticmethod
    def save_user_name(telegram_id, name, key):
        query = Users.get(Users.telegram_id == encrypt(telegram_id, key))
        query.name = encrypt(name, key)
        query.save()

    # Сохраняет фамилия пользователя
    @staticmethod
    def save_user_surname(telegram_id, surname, key):
        query = Users.get(Users.telegram_id == encrypt(telegram_id, key))
        query.surname = encrypt(surname, key)
        query.save()

    # Сохраняет отчество пользователя
    @staticmethod
    def save_user_patronymic(telegram_id, patronymic, key):
        query = Users.get(Users.telegram_id == encrypt(telegram_id, key))
        query.patronymic = encrypt(patronymic, key)
        query.save()
    ...
    @staticmethod
    def get_user_next_question(telegram_id, key):  # Очередной неотвеченный вопрос
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
    ...
    @staticmethod
    def get_next_user_question_and_answers(telegram_id, key):
    ...
    @staticmethod
    def get_user_tests(telegram_id, key):
        query1 = Users.select(Users.id).where(Users.telegram_id == encrypt(telegram_id, key))
        if len(query1) == 0:
            return -1
        user_id = query1[0].id
    ...
    @staticmethod
    def get_user_result(telegram_id, test_num, key):
        user_tests = Requests.get_user_tests(encrypt(telegram_id, key))
        if user_tests != -1:
    ...
    @staticmethod
    def get_user_specialists(telegram_id, test_num, key):
        specialists_ids = Requests.get_user_result(encrypt(telegram_id, key), test_num)
        specialists_info = Requests.get_specialists_info(specialists_ids)
        return specialists_info

    @staticmethod
    def get_user_info(telegram_id, key):
        query = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        if len(query) != 0:
            user = query[0]
            info = [0 for _ in range(5)]
            info[0] = decrypt(user.name, key)
            info[1] = decrypt(user.surname, key)
            info[2] = decrypt(user.patronymic, key)
            info[3] = user.sex
            info[4] = user.b_date
            return info
        else:
    ...
    @staticmethod
    def start_new_test(telegram_id, text_info="", key):
        query1 = Users.select().where(Users.telegram_id == encrypt(telegram_id, key))
        user_id = query1[0].id
