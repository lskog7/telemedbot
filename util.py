...
Я создан на хакатоне в рамках IV Открытой конференции молодых ученых Центра диагностики и телемедицины (03-04 апреля 2024 г.)'''
    ...
    @staticmethod
    def start_text():
        text = '''Подчеркиваю важность честных ответов. Они помогут нам точно оценить ваше состояние и предложить наилучшее решение для вашего здоровья.'''
        return text

    @staticmethod
    def result_text(doctor_list):
        if not doctor_list:
            text = '''Спасибо за вашу откровенность и ответы на вопросы. На основе предоставленной информации, мы пришли к выводу, что ваше состояние не требует обращения к специалистам на данный момент.

Если у вас возникнут новые симптомы, не стесняйтесь обращаться к нам.'''
            return text
        else:
            text = '''На основе предоставленной информации я рекомендуем вам обратиться к следующим специалистам:'''
            for doctor in doctor_list:
                text += f'\n🧑‍⚕️ {doctor}'
            text += '''\n\nМы также подготовили форму с вашей предварительной информацией, которая будет предоставлена врачу для более эффективного приема.'''
            return text
